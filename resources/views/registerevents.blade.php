...
<head>
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
    ...
    <link rel="stylesheet" href="{{ asset('/css/cadastrar_evento.css')}}">
</head>

<body style="background-image: url({{ asset('/img/img.jpg')}}); background-size: cover; background-repeat: no-repeat">
<header>
<nav class="navbar">
        <a style="color: white; font-weight: 500 background-color: rgb(0, 0, 0);
    font-size: 16px;" class="logo" href="{{ route('home') }}" >EvEnter</a>
        </>
        </nav>
        <div class="botoes">
    ...
    <div class="box">
        <form method="POST" action="{{ route('registerevent')}}"> 
        @csrf
